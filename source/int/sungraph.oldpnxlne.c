...
/*
		offsets[ i ] = ( char * )( scrbm + scrbm[ OFFSET ] ) - ( char * ) storeBase ;
		offsets[ i ] = ( char * )( scrbm + scrbm[ OFFSET ] + i * ( slinc / 4 ) * Yscr ) ;
*/
...
pnxlne() /* standard PNX drawline on bitmap or window, no last point */
{
	...
	col = *sp++ ; 		/* A number between 0 and 255 */
...
*/
	...
	while ( nplanes-- > ( psint ) 0 )			/* do draw.line on every plane */
	{
		...
		ptr = mkptr( wdes_d[ nplanes + 3 ] ) ;

/*
		rrule = ( col >> nplanes ) & 1 ? SAMROP_DST | SAMROP_NOT( SAMROP_DST ) : SAMROP_DST & SAMROP_NOT( SAMROP_DST ) ;
*/
		rrule = SAMROP_SET ;
	...
	if ( wst[ WINDOW ] )				/* if its a draw the vector on the screen */
	{
		...
		if ( start + nplanes <= Zscr && Xscreen != ( psint ) -1 )
		{
