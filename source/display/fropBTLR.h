/* a scanline rasterop copying data from bottom to top and left to right */
static void ROPFUNC( unsigned int *dst,int dslw,int dx,int dy,int w,int h,unsigned int *src,int sslw,int sx,int sy )
{
	...
	src += sw0 ; src += sy * sslw ; src += h * sslw ;		/* set src to scan line after bits to rop */
	dst += dw0 ; dst += dy * dslw ; dst += h * dslw ;		/* set dst to scan line after bits to rop */
